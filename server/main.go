...
func main() {
	...
	e.Use(middleware.CORSWithConfig(middleware.CORSConfig{
		AllowOrigins:     []string{"https://labstack.com", "https://labstack.net", "http://localhost:8080", "http://localhost:5173"},
	...
	e.POST("/login", func(c echo.Context) error {
		...
		c.Request().AddCookie(&http.Cookie{
			Name:  "session",
			Value: sess.Values["loginCompleted"].(string),
			Path:  "/",
		})
		sess.Save(c.Request(), c.Response())
		text := "name: " + name + ", pass: " + pass
		// テンプレートに渡す値
